...
account_id = os.getenv('ACCOUNT_ID')  # Add the account ID from the environment variables

def authenticate_user(url, login, password, app_token, account_id=""):
    payload = {
        "header": {
            "app_token": app_token,
            "api_version": "1.2",
            "msg_key": f"auth.user.login.{str(uuid.uuid4())}",  # Generating a unique message key
